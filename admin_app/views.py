...
from admin_app.forms import WorksForm, BackgroundForm
...
class AccessModelView(ModelView):
    ...
    def update_model(self, form, model):
        ...
        super(AccessModelView, self).update_model(form, model)
    ...
    def delete_model(self, model):
        ...
        super(AccessModelView, self).delete_model(model)
        delete_mp4(content_path, encoded_name, usage)


class OneBackgroundView(AccessModelView):
    column_list = ['order_num', 'first_name', 'second_name','start', 'length', 'video_url', 'category']
    form = WorksForm

class AllBackgroundView(AccessModelView):
    form = BackgroundForm
    column_list = ['order_num', 'first_name', 'second_name','start', 'length', 'video_url']

class NewStuffView(OneBackgroundView):
    pass
class CommercialsView(OneBackgroundView):
    pass

class MusicVideosView(OneBackgroundView):
    pass

class ShortFilmsView(AllBackgroundView):
    pass

class DocumentariesView(AllBackgroundView):
    pass

class DopWorksView(AllBackgroundView):
    pass
